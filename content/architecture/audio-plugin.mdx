...
async function normalizeAudio(
  ...
  await execFFmpeg([
    '-i', input,
    '-af', `loudnorm=I=${options.loudness}:TP=${options.truePeak}:LRA=${options.lra}`,
    '-ar', '48000',
    output
  ]);
}
...
const args = [
  ...
  '-id3v2_version', '3',     // ID3v2.3 for MP3
...
async function extractTrack(
  ...
  await execFFmpeg([
    ...
    '-c:a', 'copy',                // Copy without re-encode
    output
  ]);
}
```
...
async function mergeAudio(
  ...
  if (options.crossfade) {
    const filterComplex = buildCrossfadeFilter(inputs.length, options.crossfade);
