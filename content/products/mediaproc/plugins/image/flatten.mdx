...
<Terminal
  commands={[{ command: "mediaproc image flatten <input> [options]" }]}
/>
...
<Terminal
  commands={[
    { command: "mediaproc image flatten logo.png" },
    { output: "✓ Flattened with white background", isSuccess: true },
  ]}
/>

<InlineTerminal command='mediaproc image flatten logo.png --background "#2C3E50"' />
<InlineTerminal command="mediaproc image flatten transparent.png -o output.jpg" />
...
| `--quality`    | `-q`  | `number`  | -                   | Output quality (1-100) (optional). Applies to JPEG/WEBP/AVIF. For PNG, maps to compression level (higher quality = lower compression). Ignored for other formats. |
