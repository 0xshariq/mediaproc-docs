...
<Terminal
  commands={[{ command: "mediaproc image optimize <input> [options]" }]}
/>
...
<Terminal
  commands={[
    ...
    {
      output: "✓ Optimized • 2.4 MB → 1.8 MB (25% reduction)",
      isSuccess: true,
    },
  ]}
/>
...
<Terminal
  commands={[
    ...
    {
      output: "✓ Optimized • 845 KB → 712 KB (16% reduction)",
      isSuccess: true,
    },
  ]}
/>
...
<Terminal
  commands={[
    ...
    {
      output: "✓ Optimized • 3.2 MB → 1.1 MB (66% reduction)",
      isSuccess: true,
    },
  ]}
/>
...
| `--quality <quality>` | `-q`  | number  | -                         | Quality level (1-100) (optional). Only applies to JPEG, WEBP, and AVIF. For PNG, quality is mapped to compression level (0-9, higher is more compressed). Ignored for other formats. |
...
- Compression level (0-9) is mapped from quality flag (higher quality = lower compression)
...
*Quality flag applies to WebP and AVIF. For PNG, quality is mapped to compression level. For other formats, the quality flag is ignored.*
