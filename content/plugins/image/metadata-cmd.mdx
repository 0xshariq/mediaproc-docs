...
```bash
# View metadata (default mode)
mediaproc image metadata <input> [options]

# Remove metadata
mediaproc image metadata <input> --remove [options]

# Export metadata
mediaproc image metadata <input> --export <path> [options]
```
...
ℹ️  This image has no embedded metadata
```
...
# Shows: ℹ️  This image has no embedded metadata
```
...
# Multiple files
photo1.jpg → metadata-1.json
photo2.jpg → metadata-2.json
```
...
- Use consistent naming: `photo.jpg` → `photo-clean.jpg`
