...
<Terminal
  commands={[{ command: "mediaproc image pixelate <input> [options]" }]}
/>
...
<Terminal
  commands={[
    { command: "mediaproc image pixelate photo.jpg" },
    { output: "✓ Pixelated with 10px blocks", isSuccess: true },
  ]}
/>

<InlineTerminal command="mediaproc image pixelate image.png --pixels 20" />
<InlineTerminal command="mediaproc image pixelate photo.jpg -p 5" />
<InlineTerminal command="mediaproc image pixelate face.jpg --pixels 30" />
<InlineTerminal command="mediaproc image pixelate game.jpg -p 8 -o retro.jpg" />
...
| `--quality <num>` |       | number  | —                         | Set output quality for JPEG, WEBP, AVIF (1-100), or compression level for PNG (0-9). Ignored for other formats. Optional, no default. |
...
The `--quality` flag is **optional** and has no default value. It only affects output for these formats:

- **JPEG, WEBP, AVIF**: Sets output quality (1-100)
- **PNG**: Sets `compressionLevel` (0-9)
- **Other formats**: The flag is ignored

If not specified, the encoder's default quality is used for supported formats.
