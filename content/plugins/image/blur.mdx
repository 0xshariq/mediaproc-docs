...
<Terminal commands={[{ command: "mediaproc image blur <input> [options]" }]} />
...
<Terminal
  commands={[
    { command: "mediaproc image blur photo.jpg" },
    { output: "✓ Applied blur (sigma: 10)", isSuccess: true },
  ]}
/>

<Terminal
  commands={[
    { command: "mediaproc image blur portrait.jpg -s 3" },
    { output: "✓ Light blur applied (sigma: 3)", isSuccess: true },
  ]}
/>

<InlineTerminal command="mediaproc image blur background.png -s 15" />
<InlineTerminal command="mediaproc image blur screenshot.jpg -s 40" />
