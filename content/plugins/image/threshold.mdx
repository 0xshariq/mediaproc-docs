...
<Terminal
  commands={[{ command: "mediaproc image threshold <input> [options]" }]}
/>
...
<Terminal
  commands={[
    { command: "mediaproc image threshold document.jpg" },
    { output: "✓ Applied threshold (128)", isSuccess: true },
  ]}
/>

<InlineTerminal command="mediaproc image threshold scan.jpg -t 180" />
<InlineTerminal command="mediaproc image threshold image.jpg -t 80" />
<InlineTerminal command="mediaproc image threshold photo.jpg -t 128 -o binary.png" />
<InlineTerminal command='mediaproc image threshold "scans/*.jpg" -t 200' />
...
| `-q, --quality <quality>` |     | number  | _(none)_ | Output quality 1-100 (optional, only for JPEG, WebP, AVIF; for PNG, maps to compression level; ignored for others) |
| `--dry-run`           |       | boolean | `false` | Preview without making changes                        |
| `--verbose`           | `-v`  | boolean | `false` | Show detailed output                                  |
## Output Formats

- **JPEG, WebP, AVIF**: Quality can be set with `-q` (1-100). If not provided, encoder default is used.
- **PNG**: `-q` maps to compression level (higher quality = lower compression). If not provided, encoder default is used.
- **Other formats**: The `-q` flag is ignored.

> **Note:** The `--quality`/`-q` flag is optional and only affects JPEG, WebP, AVIF, and PNG outputs. For PNG, the value is mapped to compression level (0-9). For all other formats, this flag is ignored.
