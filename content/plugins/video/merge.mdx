...
<Callout type="info">
  **Auto-Detection:** The command analyzes all input videos and automatically
  selects fast concat (lossless) if formats match, or re-encode mode if they
  differ in resolution, codec, or frame rate.
</Callout>
...
<Terminal
  commands={[
    {
      command: "mediaproc video merge <input1> <input2> [input3...] [options]",
    },
  ]}
/>
...
<Callout type="tip">
  **Professional Touch:** Use 1-2 second transitions for most content. Shorter
  (0.3-0.5s) for fast-paced videos, longer (2-3s) for dramatic effect.
</Callout>
...
**Problem It Solves:**
Different videos often have different audio levels (some loud, some quiet), creating jarring volume changes during playback.
...
<Callout type="info">
  **Audio Standards:** Normalization targets -23 LUFS (Loudness Units Full
  Scale), the broadcast standard for consistent audio levels.
</Callout>

**Learn More:**

- [Understanding LUFS](https://en.wikipedia.org/wiki/LKFS)
...
- `h264` - Universal compatibility (default)
- `h265` - Better compression, modern devices
- `vp9` - WebM format, web streaming

**Audio Codecs:**

- `aac` - Best for MP4 (default)
- `mp3` - Universal compatibility
- `opus` - Best quality-to-bitrate, modern browsers
...
- `18-20` - High quality, large files
- `23` - Default, good balance
- `26-28` - Smaller files, acceptable quality
...
- `mp4` - Universal (default)
- `mkv` - Advanced features, multi-track support
- `webm` - Web streaming
...
<Terminal
  commands={[
    { command: "mediaproc video merge video1.mp4 video2.mp4" },
    { output: "Input files:" },
    { output: "1. video1.mp4 (1920x1080, H.264, 00:02:30)" },
    { output: "2. video2.mp4 (1920x1080, H.264, 00:03:15)" },
    { output: "" },
    { output: "Format check: All videos compatible" },
    { output: "Method: Fast concat (no re-encoding)" },
    { output: "" },
    { output: "Merging videos..." },
    { output: "✓ Merge complete", isSuccess: true },
    {
      output: "Output: merged.mp4 • Duration: 00:05:45 • 180.3 MB",
      isSuccess: true,
    },
  ]}
/>

<InlineTerminal command="mediaproc video merge part1.mp4 part2.mp4 part3.mp4" />
...
<Terminal
  commands={[
    ...
    { output: "✓ Merged to complete.mp4", isSuccess: true },
  ]}
/>
...
<Terminal
  commands={[
    ...
    { output: "✓ Saved to output/combined.mp4", isSuccess: true },
  ]}
/>

<Terminal
  commands={[
    { command: "mediaproc video merge video1.mp4 video2.mp4 -o output/" },
    { output: "✓ Saved to output/merged.mp4", isSuccess: true },
  ]}
/>
...
<Terminal
  commands={[
    ...
    { output: "✓ Merged 5 segments • Total: 00:15:30", isSuccess: true },
  ]}
/>

<InlineTerminal command="mediaproc video merge clip*.mp4 -o compilation.mp4" />

<InlineTerminal command="mediaproc video merge *.mp4 -o merged.mp4" />
...
<Terminal
  commands={[
    ...
    { output: "Input files:" },
    { output: "1. video.mp4 (1920x1080, H.264)" },
    { output: "2. video.avi (1280x720, MPEG-4)" },
    { output: "3. video.mkv (1920x1080, H.265)" },
    { output: "" },
    { output: "Format check: Videos have different properties" },
    { output: "Method: Re-encode mode" },
    { output: "" },
    { output: "Merging with re-encoding..." },
    { output: "✓ Merge complete (slower due to re-encoding)", isSuccess: true },
    { output: "Output: combined.mp4", isSuccess: true },
  ]}
/>

<Terminal
  commands={[
    { command: "mediaproc video merge 1080p.mp4 720p.mp4 4k.mp4 -o mixed.mp4" },
    { output: "✓ Merged different resolutions (re-encoded)", isSuccess: true },
  ]}
/>

<InlineTerminal command="mediaproc video merge h264.mp4 h265.mp4 -o merged.mp4" />
...
<Terminal
  commands={[
    ...
    { output: "[DRY RUN] Would merge 3 files:" },
    { output: "1. video1.mp4 (00:02:30, 1920x1080, H.264)" },
    { output: "2. video2.mp4 (00:01:45, 1920x1080, H.264)" },
    { output: "3. video3.mp4 (00:03:00, 1920x1080, H.264)" },
    { output: "" },
    { output: "Method: Fast concat (all files compatible)" },
    { output: "Total duration: 00:07:15" },
