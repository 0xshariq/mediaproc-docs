...
<Terminal commands={[{ command: 'mediaproc video transcode <input> [options]' }]} />
...
<Terminal commands={[
  ...
  { output: '✓ Transcoded to H.265 (50% better compression)', isSuccess: true }
]} />
...
<Terminal commands={[
  ...
  { output: '✓ Transcoded with Opus audio (excellent quality)', isSuccess: true }
]} />
...
<Terminal commands={[
  { command: 'mediaproc video transcode input.avi -f mp4 --bitrate 3000k --audio-bitrate 192k' },
...
<Terminal commands={[
  { command: 'for file in *.avi; do mediaproc video transcode "$file" -f mp4 -o converted/; done' },
