...
<Terminal commands={[{ command: 'mediaproc video extract-audio <input> [options]' }]} />
...
<Terminal commands={[
  ...
  { output: 'Output: video.mp3 • Format: MP3 • 192 kbps • 5.4 MB', isSuccess: true }
]} />
...
<Terminal title="Podcast from video" commands={[
  { command: 'mediaproc video extract-audio interview.mp4 -f mp3 --bitrate 128k -o podcast.mp3' },
  { output: '✓ Podcast created from interview', isSuccess: true }
]} />

<Terminal title="Music extraction" commands={[
  { command: 'mediaproc video extract-audio music-video.mp4 -f mp3 --bitrate 320k -o song.mp3' },
  { output: '✓ High-quality music extracted', isSuccess: true }
]} />

<Terminal title="Voice extraction" commands={[
  { command: 'mediaproc video extract-audio lecture.mp4 -f opus --bitrate 64k -o lecture-audio.opus' },
  { output: '✓ Voice optimized audio extracted', isSuccess: true }
]} />

<Terminal title="Batch audio extraction" commands={[
  { command: 'for file in *.mp4; do mediaproc video extract-audio "$file" -f mp3 -o audio/; done' },
...
<Terminal commands={[{ command: 'mediaproc video extract-frames <input> [options]' }]} />
